...
#include <sphys/collision/HalfEdgeMeshRaycast.h>
...
TEST(MeshRaycast, closestHitFalse)
{
	...
	const Ray ray1({ 1.238742113f, 3.651285171f, 3.549336910f }, { -0.617665886f, -0.526446342f, -0.584245741f });

	HalfEdgeMeshRaycast<8> raycast(kTolerance);
	raycast.buildKDTree(&mesh, &normals);
	auto result = raycast.closestHit(ray1);
...
{
	auto [mesh, normals] = createTestMesh1();
	const Ray ray1({ -3.767188549f, -3.157658576f, 1.147563099f }, { 0.619586884f, 0.732411921f, 0.282284826f });
...
{
	auto [mesh, normals] = createTestMesh1();
	const Ray ray1({ 1.445234775f, -4.599827766f, 0.190760493f }, { -0.412362039f, 0.875649988f, -0.251385301f });
...
{
	auto [mesh, normals] = createTestMesh1();
	const Ray ray1({ 1.093121886f, -0.150672316f, 0.190760493f }, { -0.180637240f, 0.056110482f, 0.981948018f });
...
{
	auto [mesh, normals] = createTestMesh1();
	const Ray ray1({ 0.006464734f, -0.150672316f, 0.190760493f }, { 0.146256104f, -0.019927343f, 0.989045977f });
	const int expectedFace = 1;
